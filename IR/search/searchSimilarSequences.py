...
def IR_Method(sequencesDatabase, inputSequence, result, times, tokenizationMethod, similarityMethod, TF_method,
              IDF_method, preprocessed=0, processed_sequences_database=[], numberOfOutputs=3,
              numberOfSequencesToSearch=100, TF=1, IDF=0, epsilon=2, operator='KNN'):
    ...
    # If multiset / vector
        ...
        if (len(processed_sequences_database) == 0):
            ...
            for elements in processed_sequences_database:
                processed.append(
                    TFIDF.updateWeights(elements, processed_sequences_database, TF_method, IDF_method, TF=TF, IDF=IDF))
